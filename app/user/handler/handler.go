...
func (h *UserHandler) GetUserInfo(c *gin.Context) {
	...
	if !isExist {
		log.Logger.Error("User ID not exist")
		c.JSON(http.StatusUnauthorized, gin.H{
	...
	if !ok {
		log.Logger.Error("Error at converting")
		c.JSON(http.StatusUnauthorized, gin.H{
			"error_message": "Invalid format ID",
		})
		return
	}

	user, err := h.UserUsecase.GetUserByID(c, int64(userID))
	if err != nil {
		log.Logger.Errorf("h.UserUsecase.GetUserByID got an error at %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error_message": err.Error(),
		})
		return
	...
	if user.ID == 0 {
		log.Logger.Error("Cannot found user")
		c.JSON(http.StatusInternalServerError, gin.H{
